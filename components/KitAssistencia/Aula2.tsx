/**
 * Variante da aula SEM trava (/kit-aula-2).
 *
 * A /kit-aula original só mostra a oferta depois de 4 min creditados pelo
 * servidor. Isso qualifica, mas quase ninguém chega lá: com o mesmo tráfego,
 * a versão travada converteu 2% contra 13,8% da página aberta. Aqui a oferta
 * nasce visível e o vídeo é convite, não pedágio.
 *
 * O que muda em relação à Aula.tsx:
 *   · `oferta` já vem preenchida, então `liberado` é sempre true e os ramos
 *     de trava do JSX nunca renderizam (ficaram como estão, inertes);
 *   · /api/aula não é mais chamada — não há tempo a creditar;
 *   · o player começa depois da vinheta e não bloqueia mais o avanço.
 */
...
// A vinheta de abertura ocupa os 8 primeiros segundos. Começar depois dela
// põe o Ed falando já no primeiro frame que a pessoa vê.
const COMECA_EM = 8;

// Não trava mais nada: sobrevive só como marco de audiência qualificada.
const LIBERA_EM = 240;
...
// Espelha o OFERTA de functions/api/aula.ts. Lá só era emitida com os 4 min
// fechados; aqui é pública desde o primeiro frame.
const OFERTA_ABERTA: Oferta = {
    selo: 'Condição desta semana',
    de: 'R$ 2.000',
    por: '997',
    termos:
    ...
    whatsapp:
        'https://wa.me/5592985146299?text=' +
        encodeURIComponent(
            'Oi Ed! Vi a aula do Kit Assistência Técnica Plus e quero montar a minha.',
        ),
};
...
const Aula: React.FC = () => {
    ...
    const [oferta, setOferta] = useState<Oferta | null>(OFERTA_ABERTA);
    ...
    useEffect(() => {
        // Sem trava não há sessão a abrir nem tempo a creditar. O restante
        // deste efeito continua abaixo apenas para manter o arquivo próximo
        // da Aula.tsx original e facilitar comparar as duas.
        return;
    ...
    const onTime = useCallback(() => {
        ...
        // Quem assinava AulaAssistida era o servidor, ao fechar os 4 min.
        // Sem ele, o próprio player emite — segue valendo como sinal de
        // audiência qualificada, só não vale mais como pedágio.
        if (maiorRef.current >= LIBERA_EM) {
            rastrear(
                'AulaAssistida',
                { content_name: 'Kit Assistência Técnica Plus', segundos: LIBERA_EM },
                { umaVezSo: true },
    ...
    // Descarta a vinheta: a reprodução nasce nos 8s. maiorRef acompanha
    // pra que o marco de 25% não conte o trecho que ninguém viu.
    const aoCarregar = useCallback(() => {
        const v = videoRef.current;
        if (!v) return;
        if (v.currentTime < COMECA_EM) v.currentTime = COMECA_EM;
        if (maiorRef.current < COMECA_EM) maiorRef.current = COMECA_EM;
        setPronto(true);
    }, []);

    // Mantido sem efeito: sem trava, pular é escolha de quem assiste.
    const onSeeking = useCallback(() => {
        const v = videoRef.current;
        if (!v) return;
        // (o anti-pulo da versão travada saía daqui)
        void v;
    }, []);
            ...
            <main className="max-w-[1000px] mx-auto px-5 md:px-8 py-8 md:py-12">
                ...
                <div className="text-center">
                    ...
                    <p className="text-[15px] md:text-[17px] leading-relaxed max-w-2xl mx-auto mb-7" style={{ color: MUTED }}>
                        Em 4 minutos eu abro a estrutura inteira e mostro ela funcionando
                        na tela. A condição desta semana está logo abaixo.
                    </p>
                </div>
